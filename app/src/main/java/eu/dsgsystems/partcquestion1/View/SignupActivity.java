...
public class SignupActivity extends AppCompatActivity {
    ...
    public void signup() {
        ...
        int flag=0;
        if(dataController.loadUsers() != null) {
            users = dataController.loadUsers();
            for (User muser : users) {
                if (muser.getEmail() == email) flag = 1;
            }
        }
        Date date = new Date();
        if(flag == 0) {
            user = new User(name, address, email, mobile, password, date.toString(), new ArrayList<MyMarker>());
            dataController.addUser(user);
        } else {
            Toast.makeText(getBaseContext(), "User with this email already exist.", Toast.LENGTH_LONG).show();
        }
